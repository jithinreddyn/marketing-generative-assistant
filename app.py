...
for k, v in [("history", []), ("brief", None), ("media_bytes", None),
...
def main():
    ...
    if go:
        if not st.session_state.api_key:
            ...
            # Step 2 — Generate image or video via Pollinations
            if gen_prompt:
                if prompt_type == "video":
                    chosen_model = VIDEO_MODELS[video_model_label]
                    with st.spinner(f"Generating video with {chosen_model} via Pollinations.AI… (60–120s, please wait)"):
                        vid, err = generate_video(gen_prompt, model=chosen_model, duration=video_duration)
                        st.session_state.media_bytes = vid
    ...
    if st.session_state.brief:
        ...
        if st.session_state.media_bytes:
            st.markdown("<br>", unsafe_allow_html=True)
            if kind == "video":
                # Write to a named temp file — required for Windows + st.video
                suffix = ".mp4"
                with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
                    tmp.write(st.session_state.media_bytes)
                    tmp_path = tmp.name
                try:
                    st.video(tmp_path)
                except Exception:
                    st.info("Preview unavailable — download the file to watch it.")
                finally:
                    try:
                        os.unlink(tmp_path)
                    except Exception:
        ...
        elif st.session_state.gen_prompt:
            err_msg = getattr(st.session_state, "video_error", "") or ""
            if err_msg:
                st.warning(f"⚠️ {err_msg}")
            else:
                st.warning(
                    "Generation timed out or returned no file. "
                    "Try `wan-fast` model, or paste the prompt above at pollinations.ai manually."
                )
